...
$$
\lambda_i = \exp(X_i\beta) = \exp(\mu_i) \Rightarrow 0 < \lambda
$$
...
```{r poisson-5, warning=FALSE, out.width="100%"}
...
summary(Y)
ggplot(as.data.frame(Y), aes(x=Y)) + geom_histogram(bins = 10)
```
...
Program the Poisson log-likelihood function (cf. slide 13)
...
## Eck and Hultman 2007: One-sided violence against civilians 
Load the eck_rep.dta replication dataset for @eck_hultman_2007
...
Let L1 be the likelihood for the null model with m parameters and L2 be the likelihood for the alternative model with n parameters, n > m. One can show that when $n \to \infty$, $-2 \times log(\frac{L1}{L2})$ converges  to the $\chi^2$ distribution with $k = n - m$ degrees of freedom. See also @degroot_schervish_2012: pp. 543--544.
...
*Now we need an additional step:* 

Plug the lambda and theta into the negative binomial distribution. And then we average over the fundamental uncertainty for expected values. Please have a look at the @king_et_al_2000 article from Week 7 again for further information. 
...
# print plot
plot + scale_color_viridis(discrete = T) & theme(legend.position = "right")
```
