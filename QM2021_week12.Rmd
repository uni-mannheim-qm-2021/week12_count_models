...
$$
\Pr(Y|\lambda) = \prod_{i=1}^{N}    \frac{e^{-\lambda_i}\lambda_i^{y_{i}}}{y_{i}!} 
$$
...
$$
\ln L(\lambda|Y) = \ell(\lambda|Y) \propto \sum_{i=1}^{n} (y_{i} ln(\lambda_i) - \lambda_i)
$$

Replacing $\lambda$ with the reparametrized linear predictor $\lambda = e^{\mu_i} = e^{X_{i}\beta}$ gives us the likelihood function of our model:

$$
\ell(\beta|Y) = \ln L(\beta|Y) \propto \sum_{i=1}^{n} (y_{i} X_{i}\beta -e^{X_{i}\beta})\\
$$
...
We can also use Likelihood ratio test for other examples of nested models, for instance to examine the differences in the specification of teh systematic component.
